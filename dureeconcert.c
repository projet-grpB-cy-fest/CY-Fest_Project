...
int duree_activite(struct tm debut, struct tm fin, struct tm instant_t){//Indique la fin des reservation, retourne 1 si le concert est fini/commencé, dans ce cas aucune réservation n'est possible puisque le concert est fini/commencé. Sinon retourne 0.
//instant_t pourrait étre la valeur entrez par l'utilisateur pour indiquer l'heure de reservation 
    if(instant_t.tm_hour>=fin.tm_hour || instant_t.tm_hour<=debut.tm_hour){
        printf("fin");
        return 1;
    }
    else{
...
int main( ){
    ...
    duree_activite(date_debut,date_fin,instant);

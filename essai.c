...
printf("Nombre de sièges réservés : %d\n", salles[i].nb_siege_reserves);
printf("Prix des sièges (avant/milieu/arrière) : %d / %d / %d euros\n\n",
salles[i].prixSieges[0], salles[i].prixSieges[1], salles[i].prixSieges[2]);
}
}

void gestionFestivalier(Salle salles[]) {
afficherListeSalles(salles, NB_SALLES);

// Demande à l'utilisateur de choisir une salle
int choix;
printf("Dans quelle salle voulez-vous aller ? (1-%d) : ", NB_SALLES);
while (scanf("%d", &choix) != 1 || choix < 1 || choix > NB_SALLES) {
    printf("Choix invalide. Veuillez choisir une salle valide (1-%d) : ", NB_SALLES);
    while (getchar() != '\n')
